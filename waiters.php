...
<html>
    ...
    <body>
        <?php
         ...
         $date = $_POST['date'];
         $loc = $_POST['loc'];
         $waiters = $_POST['waiter'];
             
         $today = "select curdate() as date";
         $havana = mysql_query($today);
         
         $tday = mysql_fetch_assoc($havana);
         $rdate = $tday['date'];
         ...
         $fed = "select itemname as item,SUM(qtyout*mult) As qty,SUM(qtyout*mult*unitprice) as extended from trans where location = '$loc' and waiter = '$waiters' GROUP BY itemname";
         
         
         
         $sfed = "select itemname as item,qtyout ,qtyout*unitprice*mult  as extended from trans where location = '$loc' And waiter = '$waiters'   ";
         
         $jfed = "select itemname as item,qtyout ,SUM(qtyout*unitprice *mult) as extended from trans where  location = '$loc' And waiter = '$waiters' GROUP BY itemname";
